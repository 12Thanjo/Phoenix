...
#define PH_LOG_FATAL 4
	...
	#define ph_internal_log(x) if(Phoenix::CMD::level <= 0){ std::cout << Phoenix::CMD::style.dim << Phoenix::CMD::get_time() << " <Phoenix> " << x << Phoenix::CMD::style.reset << "\n"; }
	...
	#define ph_log(x) if(Phoenix::CMD::level <= 0){ std::cout << Phoenix::CMD::style.dim << Phoenix::CMD::get_time() << " " << x << Phoenix::CMD::style.reset << "\n"; }
	...
	#define trace() std::cout << Phoenix::CMD::get_time() << "<Trace> File: " << __FILE__ << "\n                Line: " << __LINE__ << "\n";

	#define ph_internal_assert(x, y) if(!(x)){ ph_internal_error(y); trace(); }
	#define ph_assert(x, y) if(!(x)){ ph_error(y); trace(); }
#else
	#define ph_log_level(x)

	#define ph_internal_log(x)
